...
namespace BookingAppDio.Flight.API.Application.GetFlightById
{
    public class GetFlightByIdQueryHandler : IQueryHandler<GetFlightByIdQuery, FlightResponseDto>
    {
        ...
        public async Task<FlightResponseDto> Handle(GetFlightByIdQuery request, CancellationToken cancellationToken)
        {
            var flight = await _flightDbContext.Flights.SingleOrDefaultAsync(x => x.Id == request.Id && !x.IsDeleted, cancellationToken);
